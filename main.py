...
def diary_sender2(message):
    ...
    if is_within_boundaries('08:50:00', '15:50:00', current_time) == False:
        message_txt = 'Броускі чіл. Зараз НЕМАЄ пар'
    for i in range(len(time_data)):
        if is_within_boundaries(time_data[i][0], time_data[i][1], str(current_time)):
           if 'перерва' in time_names[i].lower():
               
                message_txt = 'Зараз - '+ str(time_names[i]) + '\n' + 'Наступна півпара/пара буде через - ' + str(time_difference(current_time, time_data[i][1]))
                ...
                if 'четверта' not in time_names[i] and 'друга' not in time_names[i] and 'маленька' in time_names[i + 1].lower():
                    ...
                    message_txt = 'Зараз - '+ str(time_names[i]) + '\n' + 'Наступна маленька перерва буде через - ' + str(time_difference(current_time, time_data[i][1])) 
                    
                elif 'четверта' not in time_names[i] and 'друга' not in time_names[i] and 'велика' in time_names[i + 1].lower():
                    message_txt = 'Зараз - '+ str(time_names[i]) + '\n' + 'Наступна велика перерва буде через - ' + str(time_difference(current_time, time_data[i][1])) 
                else:
                      message_txt = 'Зараз - '+ str(time_names[i]) + '\n' + 'Закінчення пари через - ' + str(time_difference(current_time, time_data[i][1]))
